...
BACKEND_IMAGE="${IMAGE_NAMESPACE}/chronogeolab-backend"
FRONTEND_IMAGE="${IMAGE_NAMESPACE}/chronogeolab-frontend"
...
if ! docker buildx inspect cgl-builder >/dev/null 2>&1; then
  docker buildx create --name cgl-builder --use >/dev/null
else
  docker buildx use cgl-builder
fi
